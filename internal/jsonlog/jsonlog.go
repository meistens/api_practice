...
// define level type to rep. severity of log entry
type Level int8

// init. consts which rep. security levels
const (
	LevelInfo Level = iota // default 0, others will be incremented by 1
	LevelError
	LevelFatal
	LevelOff
)

// return readable - to human brain - string for severity level
...
// define custom logger type which holds the output dest for log entries
type Logger struct {
	out      io.Writer
	minLevel Level
	mu       sync.Mutex
...
// severity level to a specified output dest
...
// declare some helpers for writing for different levels
...
func (l *Logger) print(level Level, message string, properties map[string]string) (int, error) {
	// if severity level of log entry is below minimum, nothing is done
	if level < l.minLevel {
		return 0, nil
	}
